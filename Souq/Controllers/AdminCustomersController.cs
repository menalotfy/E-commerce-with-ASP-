...
namespace Souq.Controllers
{
    ...
    public class AdminCustomersController : Controller
    {
        ...
        public ActionResult Search(string CustomerName, int? Page)
        {
            var cust = (from p in applicationDbContext.Users
                       where (p.Roles.Any(r => r.RoleId == "3") && p.UserName.Contains(CustomerName ) && p.Status == false)
        ...
        public ActionResult Index(int? Page)
        {
            var customer = (from u in applicationDbContext.Users
                        where (u.Roles.Any(r=>r.RoleId == "3") && u.Status == false)
